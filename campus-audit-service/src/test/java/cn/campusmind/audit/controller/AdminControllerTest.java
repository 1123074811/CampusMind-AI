...
class AdminControllerTest {
    ...
    void dashboardReturnsSeededAdminData() throws Exception {
        mockMvc.perform(get("/api/admin/dashboard").header("Authorization", adminToken()))
                ...
                .andExpect(jsonPath("$.data.eventTotal").value(5))
                .andExpect(jsonPath("$.data.eventPage").value(0))
                .andExpect(jsonPath("$.data.eventPageSize").value(20))
    ...
    @Test
    void adminCanCreateUpdateAndPauseDataSource() throws Exception {
        String source = """
                {
                  "name":"研究生院通知",
                  "sourceType":"PUBLIC_WEB",
                  "baseUrl":"https://graduate.example.edu.cn/notices",
                  "crawlIntervalSeconds":1800,
                  "parserType":"WEBMAGIC",
                  "selectorConfig":"{\\"itemSelector\\":\\".notice\\"}",
                  "enabled":true
                }
                """;
        String response = mockMvc.perform(post("/api/admin/sources")
                        ...
                        .content(source))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.name").value("研究生院通知"))
                .andExpect(jsonPath("$.data.enabled").value(true))
                .andReturn().getResponse().getContentAsString();
        long id = new com.fasterxml.jackson.databind.ObjectMapper().readTree(response).at("/data/id").asLong();

        mockMvc.perform(put("/api/admin/sources/" + id + "/enabled")
                        .header("Authorization", adminToken())
                        .contentType("application/json")
