...
CALL add_init_fk_if_missing('fk_event_audit_log_event', 'ALTER TABLE event_audit_log ADD CONSTRAINT fk_event_audit_log_event FOREIGN KEY (event_id) REFERENCES campus_event(id)');
CALL add_init_fk_if_missing('fk_event_audit_log_operator', 'ALTER TABLE event_audit_log ADD CONSTRAINT fk_event_audit_log_operator FOREIGN KEY (operator_id) REFERENCES user(id)');
CALL add_init_fk_if_missing('fk_crawl_task_source', 'ALTER TABLE crawl_task ADD CONSTRAINT fk_crawl_task_source FOREIGN KEY (source_id) REFERENCES data_source(id)');
CALL add_init_fk_if_missing('fk_import_task_user', 'ALTER TABLE import_task ADD CONSTRAINT fk_import_task_user FOREIGN KEY (user_id) REFERENCES user(id)');
