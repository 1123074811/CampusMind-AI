...
class SearchControllerTest {
    ...
    void setUp() throws Exception {
        try (Connection connection = dataSource.getConnection();
            ...
            statement.execute("INSERT INTO user_consent_record (user_id, consent_type, granted) VALUES (1, 'PERSONALIZATION', 1)");
        }
    }
    ...
    @Test
    void semanticSearchMapsCrawlerInformationVectorId() throws Exception {
        insertInformationItem(91L, "研究生复试调剂公告", "软件学院", "模型生成的调剂要点");
        when(decisionClient.plan(anyString(), anyList(), anyBoolean()))
                .thenReturn(new DecisionPlan("SEMANTIC_SEARCH", List.of(), "ANY", List.of(), true, false, 10));
        when(vectorSearchFeignClient.search(org.mockito.ArgumentMatchers.anyMap()))
                .thenReturn(ApiResponse.ok(new VectorSearchFeignClient.VectorSearchResult(List.of(
        ...
        mockMvc.perform(get("/api/v1/search")
                ...
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.mode").value("SEMANTIC"))
                .andExpect(jsonPath("$.data.items[0].id").value(91))
    ...
    private void insertInformationItem(long id, String title, String sourceName, String aiSummary) throws Exception {
        try (Connection connection = dataSource.getConnection();
             PreparedStatement statement = connection.prepareStatement("""
                     INSERT INTO information_item (
                       id, source_id, source_name, title, publish_time, fetched_at, detail_content,
                       item_status, parse_status, ai_status, ai_event_type, ai_summary
                     ) VALUES (?, 1, ?, ?, TIMESTAMP '2026-07-09 09:00:00', TIMESTAMP '2026-07-09 10:00:00',
                       '调剂公告完整正文', 'ACTIVE', 'DETAIL_SUCCESS', 'SUCCESS', 'NOTICE', ?)
                     """)) {
            statement.setLong(1, id);
            statement.setString(2, sourceName);
            statement.setString(3, title);
            statement.setString(4, aiSummary);
            statement.executeUpdate();
        }
    }
