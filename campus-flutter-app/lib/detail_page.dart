...
class _PrototypeDetailPageState extends State<PrototypeDetailPage> {
  ...
  Future<void> _loadDetail() async {
    try {
      ...
      if (!mounted) return;
      setState(() => _loadError = '详情加载失败：$error');
  ...
  Future<void> _loadRelated() async {
    try {
      final items =
          await widget.api.fetchRelatedItems(_item.id, widget.session);
      if (!mounted) return;
      setState(() {
        _relatedItems = items;
      ...
      ScaffoldMessenger.of(context)
          .showSnackBar(SnackBar(content: Text('相关信息加载失败：$error')));
    }
  }
  ...
  Future<void> _shareItem() async {
    ...
    await SharePlus.instance.share(
      ShareParams(text: text, subject: _item.title),
    );
  }
      ...
      body: SafeArea(
        child: ListView(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                Row(
                  children: [
                    _IconBtn(
                        icon: _fav ? Icons.bookmark : Icons.bookmark_outline,
                        onTap: _toggleFav,
                        active: _fav),
                    ...
                    const Row(
                      children: [
                        Icon(Icons.auto_awesome,
                            size: 15, color: AppTheme.brandInk),
                        SizedBox(width: 7),
                        Text('AI 智能摘要',
                            style: TextStyle(
                                fontSize: 12.5,
                      ...
                      children: [
                        ...
                        IconButton(
                          ...
                          onPressed: () => Clipboard.setData(
                              ClipboardData(text: _item.contentHash)),
            ...
            // Related
            if (_relatedItems.isNotEmpty || _relatedLoading) ...[
              const Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text('相关信息 · 去重融合',
                ...
                ..._relatedItems.asMap().entries.map((entry) {
                  ...
                  return Padding(
                    padding: EdgeInsets.only(
                        bottom: entry.key < _relatedItems.length - 1 ? 10 : 0),
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Icon(icon,
            size: 17, color: active ? AppTheme.brand : AppTheme.ink2),
      ),
    );
  }
}
